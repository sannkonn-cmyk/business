...
var Judge = (function () {
  ...
  function 未照合の法令(city) {
    var 済 = city.照合済み法令 || [];
    /* 区分が確定していない市町村は、どの法令に当たるかも分からないので、
       区域レベルの4法令のうち一覧が入っていないものをすべて挙げます。 */
    var 該当 = city.区分 === "未確定" ? 区域レベルの法令 : (city.該当法令 || []);
  ...
  function judge(input, data, 今日) {
    ...
    /* 区分が決まっていない市町村。振興山村・離島・半島の一覧が未投入で、
       「条件不利地域ではない（＝都市地域）」と言い切れない場合にここへ来ます。 */
    if (city.区分 === "未確定") {
      out.result = "要確認";
      out.route = "市町村判定";
      var 足りない = 未照合の法令(city);
      out.未照合法令 = 足りない;
      out.reasons.push({ label: "市町村区分", value: city.市町村名 + " の区分が確定していません" +
        (足りない.length ? "（" + 足りない.join("・") + " の区域一覧が未投入）" : "") });
      out.warnings.push("この市町村は" + (足りない.length ? 足りない.join("・") + "の一覧が投入されていないため" : "資料が足りないため") +
        "、条件不利地域に当たらないことを確かめられません。総務省又は島根県へ照会してください。");
      return out;
    }
    ...
    if (oaza.要確認理由) {
      ...
      /* 理由を注意欄にも出します。何が足りないのかが分からないと、
         使う人は次に何をすればよいか判断できないためです。 */
      var 欠け2 = 未照合の法令(city);
      if (欠け2.length) out.未照合法令 = 欠け2;
      out.warnings.push(oaza.要確認理由 + "。総務省又は島根県へ照会してください。");
      return out;
    }

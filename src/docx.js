...
/* 罫線なし3列。中央・右のセルを上下中央にすることで、
   あて先が偶数行でも敬称がまん中に来ます。 */
function toTable(names, honor, wName, cw){
  /* 長いあて先は列の中で折り返す。その折り返しも数えて波括弧の高さを決める。 */
  var lines = 0;
  names.forEach(function(n){ lines += Math.max(1, Math.ceil(cw(n) * TW_CHAR / wName)); });
  ...
  var namesCell = names.map(function(n){
    return para(lineFix + '<w:jc w:val="distribute"/>', run(n));
  }).join("");
...
function buildBody(D, cw){
  ...
  /* --- あて先 --- */
  if(D.to.length === 1){
    /* 1件のときは波括弧を出さず、右に全角1つ空けて敬称 */
    out.push(para('<w:ind w:firstLineChars="100" w:firstLine="210"/>',
                  run(D.to[0] + (D.honor ? "　" + D.honor : ""))));
    out.push(blank(""));
  }else if(D.to.length >= 2){
    var wHonor = D.honor ? Math.max(TW_CHAR, Math.ceil(cw(D.honor)) * TW_CHAR) : 0;
    /* 表全体が本文の幅からはみ出さないよう、あて先の列幅に上限をかける */
    var limit = BODY_W - TW_CHAR - (D.honor ? TW_BRACE_COL + wHonor : 0);
    var wName = groupWidth(D.to, limit);

    if(D.honor){
      out.push(toTable(D.to, D.honor, wName, cw));
    }else{
      /* 敬称なし：波括弧も敬称の列も出さない。均等割り付けだけそろえる。 */
      D.to.forEach(function(n){
        out.push(para(kintoPPr(wName, BODY_W - TW_CHAR - wName), run(n)));
      });
    }
    out.push(blank(""));
  }
